...
TOKEN-AWARE PARSER ARCHITECTURE (Alternative for complex scenarios).

This module provides ReasoningOutputParser, which extends the base
ReasoningParser to support token ID awareness for precise marker detection.
This is particularly important for models where thinking markers may span
multiple tokens.

When to use this parser:
- Markers may span multiple tokens (preventing partial tag leakage)
- Token IDs are available and stable
- Need precise streaming detection without guessing

When to use BaseThinkingReasoningParser instead:
- Markers are simple text strings (most models)
- Token IDs are unreliable (like Qwen3/Qwen3.5 - see WARNING below)
...
They are Russian/Thai characters in the tokenizer vocabulary.
For Qwen3/Qwen3.5, use QwenThinkParser (text-based) instead, or use
ReasoningOutputParser with token_ids=None.

Design reference: vLLM PR #34779

See also:
- vllm_mlx/reasoning/__init__.py for full architecture overview
- vllm_mlx/reasoning/think_parser.py for simpler text-based alternative
"""
...
QWEN3_MARKERS = {
    "start_token": "<think>",
    "end_token": "</think>",
    # WARNING: Token IDs 151648/151649 are NOT thinking markers!
    # They are Russian/Thai characters in Qwen tokenizer.
    # Use text-based parsing only - token IDs set to None.
    "start_token_id": None,
    "end_token_id": None,
}
...
DEEPSEEK_R1_MARKERS = {
    "start_token": "<think>",
    "end_token": "</think>",
    # DeepSeek-R1 uses similar markers (text-based only)
    "start_token_id": None,
    "end_token_id": None,
}
