...
"""
Scheduler core logic for continuous batching.

Extracted from scheduler.py as part of Phase 3 architecture refactor.

This module provides the Scheduler class that manages request scheduling
...
from .chunked_prefill import install_chunked_prefill
from .mtp import install_mtp
from .utils import CACHE_CORRUPTION_PATTERNS, is_cache_corruption_error
...
from vllm_mlx.scheduler_context import SchedulerContext, BatchGeneratorAdapter
...
class Scheduler:
    ...
    def __init__(
        ...
        # SchedulerContext: shared state container (Phase 2 refactor)
        # This eliminates circular dependencies between Scheduler and BatchGenerator
        self._context = SchedulerContext()

        # Request management - following vLLM's design
        # These now delegate to context for shared state
        ...
        # These delegate to context
        self.request_id_to_uid: Dict[str, int] = {}
        self.uid_to_request_id: Dict[int, str] = {}

        # Sync local dicts with context
        self._context.requests = self.requests
        self._context.running = self.running
        self._context.uid_to_request_id = self.uid_to_request_id
        self._context.request_id_to_uid = self.request_id_to_uid

        # BatchGenerator - the actual batching engine
        # Can be either raw BatchGenerator or BatchGeneratorAdapter
        self.batch_generator: Optional[Any] = None  # BatchGenerator or BatchGeneratorAdapter
        self._batch_adapter: Optional[BatchGeneratorAdapter] = None
    ...
    def _create_batch_generator(
    ...
    ) -> Any:  # Returns BatchGenerator or BatchGeneratorAdapter
        """
        Create a BatchGenerator with the given sampling parameters.
        
        Phase 2 refactor: Uses BatchGeneratorAdapter instead of monkey-patching.
        """
        ...
        def _prefill_progress(progress_list):
            ...
            for uid, processed, total in progress_list:
                rid = self._context.get_request_id_by_uid(uid) or "?"
                logger.info(
        ...
        # Determine if chunked prefill is needed
        ...
        if need_chunked:
            if chunked_budget <= 0:
                ...
                # still trigger via needs_boundary_split.
                chunked_budget = 999_999
            
            # Create callbacks for cache management
            mid_prefill_cb = None
            ...
            # Phase 2 refactor: Use BatchGeneratorAdapter instead of monkey-patching
            adapter = BatchGeneratorAdapter(
                batch_gen=bg,
                context=self._context,
                chunked_budget=chunked_budget,
            ...
            logger.info(f"[Scheduler] Created BatchGeneratorAdapter with chunked_budget={chunked_budget}")
            return adapter
        else:
            # No chunked prefill needed, use raw BatchGenerator
            self._batch_adapter = None
            
            # Install MTP if needed
            if self.config.enable_mtp:
                if hasattr(self.model, "mtp") and self.model.mtp is not None:
                    install_mtp(
                        bg,
    ...
    def _make_prompt_cache_save_callback(self):
        ...
        Called from ``BatchGeneratorAdapter._generation_step`` right before 
        the first output token is fed into the model. At that point 
        ``num_tokens == 0`` and the batch cache contains the exact prompt-only 
        state (correct for both KVCache and MambaCache/ArraysCache layers).

        Phase 2 refactor: Uses SchedulerContext for state access.
        """
        import time as _time

        def _prompt_cache_save(uid, extracted_cache):
            # Use context for state access (Phase 2 refactor)
            request_id = self._context.get_request_id_by_uid(uid)
            if not request_id:
                return
            request = self._context.requests.get(request_id)
    ...
    def _close_batch_generator(self) -> None:
        """Properly close BatchGenerator or BatchGeneratorAdapter."""
        # Close adapter if using it (Phase 2 refactor)
        if self._batch_adapter is not None:
            try:
                self._batch_adapter.close()
            except Exception as e:
                logger.debug(f"Error closing BatchGeneratorAdapter: {e}")
            self._batch_adapter = None
    ...
    def abort_request(self, request_id: str) -> bool:
        ...
        """
        # Use context for pending aborts (Phase 2 refactor)
        self._context.add_pending_abort(request_id)
    ...
    def _process_pending_aborts(self) -> None:
        ...
        # Use context for pending aborts (Phase 2 refactor)
        while self._context.has_pending_aborts():
            pending = self._context.get_pending_aborts()
            for request_id in pending:
                self._context.clear_pending_abort(request_id)
    ...
    def reset(self) -> None:
        ...
        # Clear context (Phase 2 refactor)
        self._context.clear()
