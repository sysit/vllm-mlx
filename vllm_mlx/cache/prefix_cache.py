...
This module provides trie-based LRU cache implementation for prefix caching.
"""
...
class PrefixCache:
    """
    Trie-based LRU prefix cache for vllm-mlx.
    ...
    Example:
        cache = PrefixCache(model, max_entries=100)

        # Check for cached prefix
        kv_cache, remaining_tokens = cache.fetch(tokens)
        if kv_cache:
            # Use cached KV, only process remaining_tokens
            pass

        # After generation, store cache for reuse
        cache.store(full_tokens, prompt_cache)
    """
    ...
    def fetch(self, tokens: List[int]) -> Tuple[Optional[List[Any]], List[int]]:
        """
    ...
    def store(self, tokens: List[int], prompt_cache: List[Any]) -> None:
        """
    ...
    def store_cache(self, tokens: List[int], prompt_cache: List[Any]) -> None:
        ...
        self.store(tokens, prompt_cache)
